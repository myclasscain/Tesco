    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <input
          ...
          v-model="nickName"
          name="nickName"
          v-validate="{ required: true, regex: /^[\u4e00-\u9fa5]{2,8}$/ }"
          :class="{ invalid: errors.has('nickName') }"
        />
        <span class="error-msg">{{ errors.first('nickName') }}</span>
      </div>
      <div class="content">
        <label>手机号:</label>
        <input
...
export default {
  ...
  methods: {
    ...
    async userRegister() {
      try {
        let { phone, code, password, isCheck, nickName } = this
        const registerData = {
          phone,
          code,
          password,
        ...
        if (success) {
          await this.$store.dispatch('user/userRegister', registerData)
