            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="skuNum"
                  @change="changeSkuNum($event.target.value * 1)"
                />
                <a
                  href="javascript:"
                  class="plus"
                  @click="skuNum < 99 ? skuNum++ : skuNum"
                  >+</a
                >
                <a
                  href="javascript:"
...
export default {
  ...
  methods: {
    ...
    // 修改商品数量
    changeSkuNum(userNum) {
      if (isNaN(userNum) | (userNum < 1)) {
        this.skuNum = 1
      } else {
        this.skuNum = this.skuNum > 99 ? (this.skuNum = 99) : parseInt(userNum)
      }
    },
    // 添加到购物车处理函数
    async addShopCart() {
      try {
        ...
        // 数据成功存到服务器之后 要将skuInfo存到本地会话空间中
  ...
  mounted() {
    this.$store.dispatch('detail/getGoodInfoById', this.$route.params.id)
  }
}
</script>
